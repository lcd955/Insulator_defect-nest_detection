...
class SyntheticFog(object):
    ...
    def fogging_img(self, img, brightness=0.7, thickness=0.06, high_efficiency = False):
        """
        fogging image
        :param img: src img
...
#     parser.add_argument('--speed_up', action='store_true', default=False, help='matrix optimization')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='synthetic_fog.py')
    parser.add_argument('--speed_up', action='store_true', default=True, help='matrix optimization')
    parser.add_argument('--source', type=str, default= r'D:\desk\yolov5\insulator_defect\InsulatorDataSet-master\datasets\images/', help='source img path')
    parser.add_argument('--save-dir', type=str, default=r"D:\desk\yolov5\insulator_defect\InsulatorDataSet-master\datasets\foggy_img/", help='output img path')
    opt = parser.parse_args()
    print(opt)
    synf = SyntheticFog()
    ...
    if os.path.isdir(opt.source):
        # Iterate over every image in the source directory
        for img_file in os.listdir(opt.source):
            ...
            if os.path.isfile(img_path):
                ...
                synf(opt.speed_up, img_path, opt.save_dir)
    else:
        print("Provided source is not a valid directory!")
